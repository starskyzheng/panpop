...
### This following script can be used to generate or remove Non-Backbone paths:
  ...
  `scripts/prase_vcf_rm_asterisk.pl` can be used to remove asterisk in the ALT colums of VCF file, and then you were able to use VG to consturct gfa file. If you needs to process NGS data by using TGS SVs generated by PanPop (in VCF format), this process is necessary. Noted this process will increase multi-allelic. Usage: `perl scripts/prase_vcf_rm_asterisk.pl --infile IN.vcf.gz -r REF.fa -o OUT1.vcf.gz`. Then using VG to consturct gfa file: `vg consturct -r REF.fa -v OUT1.vcf.gz -t 4 > OUT2.vg; vg view --vg-in --gfa OUT2.vg > OUT3.gfa`.
