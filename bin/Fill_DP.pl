...
options:
    ...
    --sid2files <FILE>     sid2files file (sid bam vcf)
...
### 2.cal_aug_dp
{
    ...
    mce_loop {
        ...
        my $cmd = "perl $scripts_dir/cal_range_depth_aug.pl --vcfposs $outpos --list_packpg $list_packpg --outdir $outdir_now -t 1 --input_format bam 2>&1 | tee $log";
        &zzsystem($cmd);
    } (@$sids);
    MCE::Loop->finish;
}
